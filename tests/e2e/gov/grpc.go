...
func (s *E2ETestSuite) TestGetParamsGRPC() {
	...
	dp := v1.NewDepositParams(params.MinDeposit, params.MaxDepositPeriod)          // we use deprecated gov commands here, but we don't want to remove them
	vp := v1.NewVotingParams(params.VotingPeriod)                                  // we use deprecated gov commands here, but we don't want to remove them
	tp := v1.NewTallyParams(params.Quorum, params.Threshold, params.VetoThreshold) // we use deprecated gov commands here, but we don't want to remove them
