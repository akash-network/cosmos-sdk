...
func ReadPersistentCommandFlags(ctx context.Context, clientCtx Context, flagSet *pflag.FlagSet) (Context, error) {
	...
	if !isOffline && clientCtx.ClientConn == nil {
		...
		if grpcURI != "" {
			...
			// do not use WithReturnConnectionError() or WithBlock as we want dial happening in the background.
			// these options makes DialContext blocking and breaks behavior of offline commands
